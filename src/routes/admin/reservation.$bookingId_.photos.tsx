...
import { TripMediaGrid } from '@/components/admin/TripMediaGrid.tsx'
import { TripMediaLightbox } from '@/components/admin/TripMediaLightbox.tsx'
...
// reservation.$bookingId.tsx, so it renders as its own page. The URL is
// unaffected: /admin/reservation/{bookingId}/photos.
export const Route = createFileRoute('/admin/reservation/$bookingId_/photos')({
    loader: async ({ params }) => {
        const [booking, media] = await Promise.all([
            getBookingById({ data: params.bookingId }),
            getTripMedia({ data: params.bookingId }),
        ])
        return { booking, media: media as TripMediaItem[] }
    },
    component: TripPhotosPage,
})

function TripPhotosPage() {
    const { booking, media } = Route.useLoaderData()
                ...
                {items.length === 0 && uploads.length === 0 ? (
                    ...
                    >
                        ...
                        <span className="max-w-sm text-center text-sm text-gray-500">
                            Document the vehicle's condition before and after the trip. Drop files here,
