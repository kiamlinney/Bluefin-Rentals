...
const birthdayCalendarClassNames = {
    ...
    caption_label:
        "inline-flex items-center gap-1 rounded-md border border-line bg-surface " +
    ...
    day_button:
        "w-9 h-9 rounded-full text-sm font-medium text-ink hover:bg-subtle transition-colors focus:outline-none cursor-pointer",
    today: "[&>button]:border [&>button]:border-ink",
    selected: "[&>button]:bg-brand [&>button]:text-on-brand [&>button]:hover:bg-pine-800",
    disabled: "[&>button]:text-ink-400 [&>button]:cursor-not-allowed [&>button]:hover:bg-transparent",
    outside: "[&>button]:text-ink-400 [&>button]:opacity-60",
    hidden: "invisible",
}
        ...
        <div ref={containerRef} className="relative">
            <button
                ...
                className={cn(
                    ...
                    hasError ? "border-red-500" : "border-line hover:border-ink-400",
            ...
            {isOpen && (
                <div className="absolute top-full left-0 mt-2 z-[200] bg-surface border border-line rounded-xl shadow-lg p-4">
                    <DayPicker
                        mode="single"
